...
//var_dump($CodValor."+".$Catego);die();
   ...
   /* public function ListadoDinamico(){
    ...
    public function ResumentProyecto(){
        ...
        if(!is_null($Resumen["key"])){
            $URIF = "http://openapp.com.ar/Proyectos/Ver/".$Resumen["key"];
            $URIT = "http://www.openapp.com.ar/Proyectos/Ver/".$Resumen["key"];

            $ReturnF = json_decode(file_get_contents('http://graph.facebook.com/?id='.$URIF));
            $Resumen["ContFacebook"] = intval($ReturnF->{'shares'});

            $ReturnT = json_decode(file_get_contents('http://urls.api.twitter.com/1/urls/count.json?url='.$URIT));
            $Resumen["ContTwitter"] = intval($ReturnT->{'count'});
        }
        $Vista = new VHome();
        $Vista->agregarFunVars(get_defined_vars());
        $Vista->VerProy();
