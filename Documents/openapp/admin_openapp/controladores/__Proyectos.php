...
class CProyectos {
    ...
    public function Export(){
      ...
      switch ($getID) {
        case 3:
            echo '"Codigo","Proyecto","Categoria","Responsable","Provincia","Mail","Telefono","Nuevos estados ","Archivo Cargado"';
            break;
        case 4:
            echo '"Codigo","Proyecto","Categoria","Responsable","Manual-1","Envio de Mail"';
            break;
        case 5:
            echo '"Codigo","Proyecto","Categoria","Responsable","Manual-2","Envio de Mail"';
            break;
        case 0:
           echo '"Codigo","Proyecto","Categoria","Responsable","Provincia","Mail","Telefono"';
           break;
        case 6:
           echo '"Codigo","Proyecto","Categoria","Responsable","Provincia","Mail","Telefono"';
           break;
        ...
        for($x=0;$x<Count($ListExport); $x++){
            foreach($ListExport[$x] as $Key => $Valor){
                ...
                if($Key == 'Categoria'){
                     ...
                     echo html_entity_decode($Categot).",";
                }
                if($Key == 'Responsable'){
                     $Responsable->abrir($Valor);
                     $Respon = $Responsable->getNombre();
                     
                     if ($getID == 4){
                          echo $Respon.",";
                     }else{
                         $Email = $Responsable->getEmail();
                         $Tel = $Responsable->getTelefono();
                         $Prov = $Responsable->getProvincia();
                         $Provincia->Abrir($Prov);
                         $Provin = $Provincia->getProvincia();
                         
                         echo $Respon.",".$Provin.",".$Email.",".$Tel.",";
    ...
    public function Listado(){
        ...
        $ListPA = $Proyectos->getProyectos(array(3, 7, 8, 9, 10), false, "FechaCargaC ASC");
       ...
       /*FILTRO ESTADO 3*/ 
        if( $getID == 3 ){
            if ($getID == 3 AND $CodEstado != "Indistinto" AND $CodEstado != "" AND $CodValor != "Indistinto" AND $CodValor != "" AND $Catego != "Indistinto" AND $Catego != ""){
                ...
                $ListPA = $Proyectos->getProyectos($CodEstado, $_SESSION["ExportWhere"], "FechaCargaC ASC");
            }
            ...
            if($getID == 3 AND $CodEstado == "Indistinto" AND $CodValor == "Indistinto" AND $Catego != "Indistinto"){ 
                ...
                $ListPA = $Proyectos->getProyectos(array($getID, 7, 8, 9, 10), $_SESSION["ExportWhere"], "FechaCargaC ASC");
            }
            ...
            if($getID == 3 AND $CodEstado != "Indistinto" AND $CodValor == "Indistinto" AND $Catego == "Indistinto"){ 
                ...
                $ListPA = $Proyectos->getProyectos(array($CodEstado), $_SESSION["ExportWhere"], "FechaCargaC ASC");
            ...
            if($CodValor == "Indistinto" AND $Catego != "Indistinto"){
                ...
                if( $getID == 2){
                   $_SESSION["ExportWhere"] = "Categoria = ".$Catego." AND Estado = ".$getID." LIMIT ".$limitInf." , ".$CantXPag;
                }else{
                    $_SESSION["ExportWhere"] = "Categoria = ".$Catego." AND Estado = ".$getID;
                    
                }
                
                if( $getID == 0){
                   $ListRechazados = $Proyectos->SQL_SELECT("*", $_SESSION["ExportWhere"]); 
                }
                
                if( $getID == 1){
                   $ListPI = $Proyectos->SQL_SELECT("*", $_SESSION["ExportWhere"]); 
                }
                
                if( $getID == 2){
                   $ListPPM = $Proyectos->SQL_SELECT("*", $_SESSION["ExportWhere"]);
                }
                
                if( $getID == 4){
                   $List180 = $Proyectos->SQL_SELECT("*", $_SESSION["ExportWhere"]); 
                }
                
                if( $getID == 5){
                   $List60 = $Proyectos->SQL_SELECT("*", $_SESSION["ExportWhere"]); 
                }
                
                if( $getID == 6){
                   $ListGanadores = $Proyectos->SQL_SELECT("*", $_SESSION["ExportWhere"]); 
                }
            }
            ...
            if($CodValor != "Indistinto" AND $Catego == "Indistinto"){
                ...
                if( $getID == 2){
                   $_SESSION["ExportWhere"] = "`Key` = '".$CodValor."' AND Estado = ".$getID." AND Categoria IN (1,2,3) LIMIT ".$limitInf." , ".$CantXPag;
                }else{
            ...
            if($CodValor != "Indistinto" AND $CodValor != ""  AND $Catego != "Indistinto" AND $Catego != ""){ 
                ...
                if( $getID == 2){
                   $_SESSION["ExportWhere"] = "`Key` = '".$CodValor."' AND Categoria = ".$Catego." AND Estado = ".$getID." LIMIT ".$limitInf." , ".$CantXPag;
                }else{
