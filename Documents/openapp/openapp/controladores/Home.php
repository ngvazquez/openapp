...
class CHome {
    public function Run(){
        ...
        $NoticiasAct = new MNoticias();
        $ListNoticias = $NoticiasAct->getGanadoresAleatorio(5);
    ...
    public function Postularme(){
        ...
        if(openapp::isLogin()){
            ...
            if (count($Responsable->SQL_SELECT("id","Email LIKE '".POST::take("user_mail")."'")) > 0){
                ...
                $Medios = new MRefMedios();
                $Medios->setResponsable($Responsable->getId());
                if (POST::take("RefMedio")){
                    $Medios->setMedio(POST::take("RefMedio"));
                }else{
                    $Medios->setMedio("web");
                }
                $Medios->Grabar();
    ...
    public function RegNewsletter(){
        $VARS = "tipo=reg".
                "&schema_listas_id=74887".
                "&listas_id=".
                "&Campo_1=".POST::take("Email").
                "&Campo_3=123456".
                "&Campo_4=".POST::take("Nombre").
                "&Campo_5=".POST::take("Apellido");

        $url = "http://app2.embluejet.com:80/Mod_Campaigns//emblue_formularios_post.asp";//.$path;
    ...
    public function MailConsulta(){
      $Email = POST::take("Email");
      $Nombre = POST::take("Nombre");
      $Asunto = POST::take("Asunto");
      $Comentario = POST::take("Comentario");
        
      openapp::EnviarMailConsulta($Email, $Nombre, $Asunto, $Comentario);                    
    }
    ...
    public function Graciasnewsletter(){
        $Vista = new VHome();
        $Vista->GracNewsletter();      
   }
