...
function Ocultar(Id){
    ...
    document.getElementById(Id).writeAttribute("onclick","Mostrar(this.id);return false;");
}

function Mostrar(Id){    
    Effect.SlideDown('Muro', {duration: 3.0});
    document.getElementById(Id).writeAttribute("onclick","Ocultar(this.id);return false;");
}
...
function CerrarLighBox(Obj,Id){
    if(ListObjCartelitos[Id] == 1 || ListObjCartelitos[Id] == undefined){
        ...
        if(Obj != undefined){
            Obj.writeAttribute("onclick","AbrirLighBox(this,'"+Id+"');");
        }
        Effect.Squish(Id);
    }else{
        AbrirLighBox(Obj, Id);
...
function AccionRespuestaPopUp(trasparent){
    return function(response){ 
       ...
       contMensaje.writeAttribute("id","contMensaje");
       mensaje.writeAttribute("id","mensaje");

        if(trasparent){
             mensaje.writeAttribute("style","background: none");
         }else{
             mensaje.writeAttribute("style","background: #fff");
         }

       TextMensajeInt.writeAttribute("id","TextMensajeInt");
       ContImgCerrar.writeAttribute("id","ContImgCerrar");
...
function AccionRespuestaPopUpConsulta(trasparent){
    return function(response){ 
       ...
       contMensaje.writeAttribute("id","contMensaje");
       mensaje.writeAttribute("id","mensajeConsulta");
       ...
       TextMensajeInt.writeAttribute("id","TextMensajeInt");
       ContImgCerrar.writeAttribute("id","ContImgCerrarConsulta");
